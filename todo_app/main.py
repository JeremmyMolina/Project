...
@app.get("/items_filter", response_model=List[TodoItem])  
def filter_items(completed: Optional[bool] = None, db: Session = Depends(get_db)):
    query = db.query(TodoItemModel)  # Создаем начальный запрос к модели задач
    if completed is not None:
        query = query.filter(TodoItemModel.completed == completed)  # Фильтруем по завершенности
    items = query.all()  # Получаем все соответствующие записи
    return items  # Возвращаем список задач
